...
public class InmuebleController : ControllerBase 
{
    ...
    {
        var result = await _repository.GetAllInmuebles();
        return Ok(_mapper.Map<IEnumerable<InmuebleResponseDto>>(result));
    }

   [HttpGet("{id}", Name = "GetInmuebleById")]
    public async Task<ActionResult<InmuebleResponseDto>> GetInmuebleById(int id)
    {
        var result = await _repository.GetInmuebleById(id);
        if(result is null)
        {
            throw new MiddlewareException(
                HttpStatusCode.NotFound,
                new { message = $"No se encontro el inmueble por este id {id}" }
            );
        }
        return Ok(_mapper.Map<InmuebleResponseDto>(result));
    }
